...
import {TransparentUpgradeableProM2MockAvsServiceManagerxy} from "@m2-openzeppelin/contracts/proxy/transparent/TransparentUpgradeableProxy.sol";
import {Script} from "forge-std/Script.sol";
import {Vm} from "forge-std/Vm.sol";
import {stdJson} from "forge-std/StdJson.sol";
import {console2} from "forge-std/console2.sol";
import {IAVSDirectory} from "@m2-eigenlayer/contracts/interfaces/IAVSDirectory.sol";
import {M2MockAvsServiceManager} from "../../src/M2MockAvsServiceManager.sol";
...
library M2MockAvsDeploymentLib {
    ...
    function deployContracts(
        ...
        M2MockAvsServiceManager mockAvsServiceManagerImpl = new M2MockAvsServiceManager(
            (IAVSDirectory(avsdirectory)),
            IRegistryCoordinator(result.registryCoordinator),
            IStakeRegistry(result.stakeRegistry),
            core.rewardsCoordinator
        );
        UpgradeableProxyLib.upgrade(result.mockAvsServiceManager, address(mockAvsServiceManagerImpl));
    ...
    function readDeploymentJson(uint256 chainId) internal returns (DeploymentData memory) {
        return readDeploymentJson("script/deployments/mock-avs/m2/", chainId);
    }
    ...
    function writeDeploymentJson(DeploymentData memory data) internal {
        writeDeploymentJson("script/deployments/mock-avs/m2/", block.chainid, data);
    }
