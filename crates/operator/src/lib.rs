...
impl Operator {
    ...
    ///
    /// * `Result<(), OperatorError>` - The result of the operation.
    pub async fn start_async<TM, F, Fut>(&self, compute_logic: F) -> Result<(), OperatorError>
    where
        TM: TaskManagerDefs,
        TM::Input:
            From<<<<TM as TaskManagerDefs>::Input as SolValue>::SolType as SolType>::RustType>,
        TM::Output: Serialize + for<'de> Deserialize<'de> + Clone,
        F: Fn(u32, TM::Input) -> Fut,
        Fut: Future<Output = Result<TM::Output, TaskManagerError>>,
    {
        let ws = WsConnect::new(&self.ws_rpc_url);
        let provider = ProviderBuilder::new()
            .on_ws(ws)
...
/// Panics if `failure_rate` is greater than 100.
#[cfg(feature = "operator-testing")]
pub fn compute_with_failures_async<Input, Output, C, F, Fut>(
    correct_logic: C,
    incorrect_logic: F,
    failure_rate: u8,
) -> impl Fn(u32, Input) -> Fut
where
    C: Fn(u32, Input) -> Fut,
    F: Fn(u32, Input) -> Fut,
    Fut: Future<Output = Result<Output, TaskManagerError>>,
    Output: SolValue + Serialize + for<'de> Deserialize<'de> + Clone,
{
    assert!(failure_rate <= 100);
