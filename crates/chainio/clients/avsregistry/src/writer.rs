...
impl AvsRegistryChainWriter {
    ...
    /// Register operator in operator sets with avs registry coordiinator
    ///
    /// # Arguments
    ///
    ...
    pub async fn register_operator_in_operator_set_with_avs_registry_coordinator(
        &self,
        ...
        // tracing info
        info!(avs_service_manager = %self.service_manager_addr, operator= %wallet.address());
        let contract_registry_coordinator =
        ...
        let encoded_params_with_socket = DynSolValue::Tuple(vec![
            ...
            DynSolValue::Array(g2_point_x),
            DynSolValue::Array(g2_point_y),
        ])
        .abi_encode_params();
        let contract_call = contract_registry_coordinator.registerOperator_1(
            wallet.address(),
            operator_set_ids,
            encoded_params_with_socket.into(),
        );
