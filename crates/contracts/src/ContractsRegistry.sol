...
contract ContractsRegistry {
    ...
    function store_test(string memory test_name, int256 index, uint256 timestamp, uint256 block_number) public {
        require(anvil_test[keccak256(abi.encodePacked(test_name))].timestamp == 0);
    ...
    function get_test_values(string memory test_name) public view returns (uint256, uint256, int256) {
        BlockAndTimestamp memory test_details = anvil_test[keccak256(abi.encodePacked(test_name))];
