...
use eigen_operator::{compute_with_failures_async, config::OperatorConfig, Operator};
use eigen_testing_utils::anvil_constants::{FIRST_ADDRESS, OPERATOR_BLS_KEY};
use incredible_squaring::{square_async, ISTaskManager};
...
async fn main() {
    ...
    let logic =
        compute_with_failures_async(square_async, |_, _| async move { Ok(U256::from(42)) }, 60);
    ...
    operator
        .start_async::<ISTaskManager, _, _>(logic)
        .await
        .unwrap();
}
