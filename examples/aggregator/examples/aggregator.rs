...
use eigen_aggregator::{
    ...
    rpc_server::ProcessSignedTaskResponseClient,
    traits::{
        task_processor::{box_error, TaskProcessor, TaskProcessorError},
        task_response::TaskResponse,
    },
    Aggregator, SignedTaskResponse,
};
...
const AGGREGATOR_IP_PORT_ADDRESS: &str = "127.0.0.1:8081";
...
async fn main() {
    ...
    let config = AggregatorConfig {
        server_address: AGGREGATOR_IP_PORT_ADDRESS.to_string(),
...
async fn send_signed_task_response(
    ...
    let transport = tarpc::serde_transport::tcp::connect(AGGREGATOR_IP_PORT_ADDRESS, Json::default)
        .await
        .unwrap();
    let client =
        ProcessSignedTaskResponseClient::new(tarpc::client::Config::default(), transport).spawn();
