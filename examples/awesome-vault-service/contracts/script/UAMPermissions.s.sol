...
contract UAMPermissions is Script {
    using AwesomeVaultDeploymentLib for *;

    address internal deployer;
    AwesomeVaultDeploymentLib.DeploymentData deploymentData;
    CoreDeploymentLib.DeploymentData internal coreData;

    function setUp() public virtual {
        deployer = vm.rememberKey(vm.envUint("PRIVATE_KEY"));
        vm.label(deployer, "Deployer");
        deploymentData = AwesomeVaultDeploymentLib.readDeploymentJson(block.chainid);
        coreData = CoreDeploymentLib.readDeploymentJson("script/deployments/core/", block.chainid);
    }

    function run() external {
        vm.startBroadcast(deployer);
        IServiceManager serviceManager = IServiceManager(deploymentData.awesomeVaultServiceManager);
        ...
        _allocationManager.setAVSRegistrar(
            deploymentData.awesomeVaultServiceManager,
        ...
        _allocationManager.updateAVSMetadataURI(
            deploymentData.awesomeVaultServiceManager, "metadataURI"
        );
