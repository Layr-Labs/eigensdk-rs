...
let key_pair :KeyPair =KeyPair::new(sk).unwrap();
println!("check if operator address is within the prime field : {:?}",U256::from_be_bytes(*LocalWallet::from_str(pvt_key).unwrap().to_bytes()) < U256::from_str("21888242871839275222246405745257275088696311157297823662689037894645226208583").unwrap());
    ...
    let tx_hash = avs_registry_writer.register_operator_in_quorum_with_avs_registry_coordinator(key_pair,digest_hash,U256::from(1719564571),quorum_nums,"65.109.158.181:33078;31078".to_string()).await.unwrap();
